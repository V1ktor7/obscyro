...
export default function NodeDataPanel({
  ...
  return (
    <div className={cn("flex flex-col gap-2", className)}>
      {recordCount > 0 ? (
        <span className="self-start rounded-full bg-gray-100 px-2 py-0.5 text-[9px] text-gray-500">
          ...
          <div className="flex flex-wrap gap-1">
            {fieldPaths.map((path) => (
              <button
                ...
                className={cn(
                  "cursor-grab rounded border border-sky-200 bg-sky-50 px-1.5 py-0.5 text-[10px] text-sky-800 active:cursor-grabbing",
  ...
  if (Array.isArray(value)) {
    ...
    return (
      <details open={depth < 1} className="group">
        <summary className="cursor-pointer text-[10px] text-gray-600 hover:text-gray-900">
